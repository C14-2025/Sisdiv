...
class TestAmortizacao:
    ...
    def test_simular_comparacao_SELIC(self):
        ...
        esperado = [
            {'tipo': 'Tesouro Selic', 'percentual_base': 1.0, 'rentabilidade_total_percentual': 21.0, 'valor_final': 1210.0},
            {'tipo': 'CDB', 'percentual_base': 1.10, 'rentabilidade_total_percentual': 23.1, 'valor_final': 1231.0},
            {'tipo': 'LCI', 'percentual_base': 1.20, 'rentabilidade_total_percentual': 25.2, 'valor_final': 1252.0}
        ]

        for r, e in zip(resultado, esperado):
            assert r['tipo'] == e['tipo']
            assert r['percentual_base'] == e['percentual_base']
            assert pytest.approx(r['rentabilidade_total_percentual'], rel=1e-3) == e['rentabilidade_total_percentual']
            assert pytest.approx(r['valor_final'], rel=1e-3) == e['valor_final']
